...
add_port_monitoring() {
	...
	echo
    echo -e "${BLUE}=== 端口专属通知接收人配置（可选）===${NC}"
    echo "可为每个端口单独配置通知接收人，留空则仅发送给管理员"
    echo "（多端口用逗号,分隔，单个值应用到所有端口）"
    echo
    read -p "Telegram Chat ID (回车跳过): " port_chat_ids_input
    read -p "企业微信 User ID (回车跳过): " port_user_ids_input
    local PORT_CHAT_IDS=()
    local PORT_USER_IDS=()
    if [ -n "$port_chat_ids_input" ]; then
        parse_comma_separated_input "$port_chat_ids_input" PORT_CHAT_IDS
        expand_single_value_to_array PORT_CHAT_IDS ${#valid_ports[@]}
        if [ ${#PORT_CHAT_IDS[@]} -ne ${#valid_ports[@]} ]; then
            echo -e "${RED}Chat ID数量与端口数量不匹配${NC}"
            sleep 2
            add_port_monitoring
            return
        fi
    fi
    if [ -n "$port_user_ids_input" ]; then
        parse_comma_separated_input "$port_user_ids_input" PORT_USER_IDS
        expand_single_value_to_array PORT_USER_IDS ${#valid_ports[@]}
        if [ ${#PORT_USER_IDS[@]} -ne ${#valid_ports[@]} ]; then
            echo -e "${RED}User ID数量与端口数量不匹配${NC}"
            sleep 2
            add_port_monitoring
            return
        fi
    ...
    for i in "${!valid_ports[@]}"; do
        ...
        local port_chat_id=""
        local port_user_id=""
        if [ ${#PORT_CHAT_IDS[@]} -gt $i ]; then
            port_chat_id=$(echo "${PORT_CHAT_IDS[$i]}" | tr -d ' ')
        fi
        if [ ${#PORT_USER_IDS[@]} -gt $i ]; then
            port_user_id=$(echo "${PORT_USER_IDS[$i]}" | tr -d ' ')
        fi
